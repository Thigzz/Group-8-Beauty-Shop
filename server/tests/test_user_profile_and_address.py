...
def admin_token(test_client, new_admin):
    ...
    test_client.post('/auth/register', data=json.dumps({
        ...
        "first_name": "Admin",
        "last_name": "User",
        "primary_phone_no": "4567890123",
        "secondary_phone_no": "3210987654"
    }), content_type='application/json')
...
def test_update_own_profile(test_client, user_token):
    """Test that a user can update their own profile and phone numbers."""
    headers = {"Authorization": f"Bearer {user_token}"}
    update_data = {
        "first_name": "Updated",
        "last_name": "Name",
        "primary_phone_no": "1112223333",
        "secondary_phone_no": "9998887777"
    }
    ...
    assert response.json['primary_phone_no'] == "1112223333"
    assert response.json['secondary_phone_no'] == "9998887777"

def test_admin_update_user_profile(test_client, admin_token, new_user):
    """Test that an admin can update another user's profile including phone numbers."""
    with test_client.application.app_context():
