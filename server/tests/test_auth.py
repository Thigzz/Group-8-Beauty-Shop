...
# Helper function to set up a user with security questions for tests
def setup_user_with_questions(test_client):
    with test_client.application.app_context():
        ...
        # Link questions to user with answers
...
def test_register_user(test_client, new_user):
    response = test_client.post('/auth/register',
                                data=json.dumps({
                                    ...
                                    "primary_phone_no": new_user.primary_phone_no,
...
def test_login_user_with_email(test_client, new_user):
    test_client.post('/auth/register', data=json.dumps({
        ...
        "first_name": "Another", "last_name": "User", "primary_phone_no": "456"
    }), content_type='application/json')
    response = test_client.post('/auth/login',
                                data=json.dumps({
                                    "login_identifier": "another@test.com",
                                    "password": "password123"
                                }),
                                content_type='application/json')
    assert response.status_code == 200
...
def test_access_protected_route(test_client, new_user):
    test_client.post('/auth/register', data=json.dumps({
        "username": "protecteduser", "email": "protected@test.com", "password": "password123",
        "first_name": "Test", "last_name": "User", "primary_phone_no": "123"
    }), content_type='application/json')
    login_res = test_client.post('/auth/login', data=json.dumps({
        "login_identifier": "protecteduser", "password": "password123"
    }), content_type='application/json')
    access_token = json.loads(login_res.data)['access_token']
    response = test_client.get('/auth/profile',
                               headers={"Authorization": f"Bearer {access_token}"})
    assert response.status_code == 200
    assert json.loads(response.data)['username'] == 'protecteduser'
...
def test_admin_access(test_client, new_admin):
    ...
    access_token = json.loads(login_res.data)['access_token']
    response = test_client.get('/auth/admin/dashboard',
                               headers={"Authorization": f"Bearer {access_token}"})
    assert response.status_code == 200
...
def test_non_admin_access_denied(test_client, new_user):
    test_client.post('/auth/register', data=json.dumps({
        "username": "nonadmin", "email": "nonadmin@test.com", "password": "password123",
        "first_name": "Non", "last_name": "Admin", "primary_phone_no": "789"
    }), content_type='application/json')
    login_res = test_client.post('/auth/login', data=json.dumps({
        "login_identifier": "nonadmin", "password": "password123"
    }), content_type='application/json')
    access_token = json.loads(login_res.data)['access_token']
    response = test_client.get('/auth/admin/dashboard',
                               headers={"Authorization": f"Bearer {access_token}"})
...
def test_get_security_questions_for_user(test_client):
    user, questions = setup_user_with_questions(test_client)
    
    response = test_client.post('/auth/reset-questions', 
                                data=json.dumps({"login_identifier": user.username}),
                                content_type='application/json')
    ...
    assert data[1]['question'] == "What is your favorite color?"

def test_verify_correct_answers(test_client):
    user, questions = setup_user_with_questions(test_client)
    
    answers = [
        {"question_id": str(questions[0].id), "answer": "Buddy"},
        {"question_id": str(questions[1].id), "answer": "Blue"}
    ]
    
    response = test_client.post('/auth/verify-answers', 
                                data=json.dumps({
...
def test_verify_incorrect_answers(test_client):
    ...
    assert response.status_code == 401
    assert b"One or more answers are incorrect" in response.data
...
def test_full_password_reset_flow(test_client):
    ...
    verify_res = test_client.post('/auth/verify-answers', 
                                  data=json.dumps({"login_identifier": user.username, "answers": answers}),
                                  content_type='application/json')
    token = json.loads(verify_res.data)['reset_token']

    reset_res = test_client.post('/auth/reset-password',
                                 data=json.dumps({
                                     "token": token,
    ...
    login_res = test_client.post('/auth/login',
                                 data=json.dumps({
                                     "login_identifier": user.username,
    ...
    assert "access_token" in json.loads(login_res.data)